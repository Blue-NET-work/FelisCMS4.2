
    <footer class="margin-bottom-10">
    	<div class="container">
        	<div class="footer">
        		<!-- left right -->
                <div class="row">
                    <div class="col-md-6">
                    	<h3><img src="{$TEMPLATES}assets/img/footerLogo.png" alt=""></h3>
                    	<p>to portal posiadający wyłączność na wszystkie prezentowane pakiety pobytowe. Co to oznacza? Takich ofert nie znajdziesz w żadnym innym serwisie</p>
                        <p>
                        	<a href="{$base_url}regulamin.html"><strong>Regulamin</strong></a>
                        </p>
                        <p>
							<a href="{$base_url}polityka-prywatnosci.html"><strong>Polityka prywatności</strong></a>
                        </p>
                    </div>
                    <div class="col-md-6">
                    	<h3 class="newsletter">Newsletter</h3>
                    	<p>Chcesz otrzymywać najświeższe informacje ze świata oraz strony internetowej? Zapisz się już teraz!</p>
						<form class="form-inline" role="form" method="POST" action="{$base_url}newsletter.html">
							<div class="input-group">
								<input type="email" class="form-control" id="newsletterEmail" placeholder="Adres e-mail">
								<span class="input-group-btn">
									<button class="btn btn-default" type="submit">zapisz się</button>
								</span>
							</div>
						</form>
                    </div>
                </div>
                <!-- ./left right -->
                <div class="row margin-top-15">
                	<div class="col-md-12 text-center">
						<a class="bottom_link" href="{$base_url}hotele.html">Obiekty</a>
		                <a class="bottom_link" href="{$base_url}okolicznosciowe.html">Okolicznościowe</a>
		                <a class="bottom_link" href="{$base_url}oferta_tygodnia.html">Oferty tygodnia</a>
		                <a class="bottom_link" href="{$base_url}nagrody.html">Nagrody</a>
		                <a class="bottom_link" href="{$base_url}pogoda.html">Pogoda</a>
		                <a class="bottom_link" href="{$base_url}kontakt.html">Kontakt</a>
                	</div>
                </div>
            </div>
        </div>
    </footer>