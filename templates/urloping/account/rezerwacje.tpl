...
{block name="content"}
            ...
            {if $rezerwacje}
                ...
                {foreach from=$rezerwacje item=item name=rezerwacje}
                	<tr>
                    	<td class="col-md-1">{html_image file="{$uploads}images/pakiety/thumb_120/{$item.pp_photo}.{$item.pp_ext}" class="img-thumbnail margin-top-10" alt="{$item.pp_alt}" title="{$item.pp_title}"}</td>
                    	<td>
