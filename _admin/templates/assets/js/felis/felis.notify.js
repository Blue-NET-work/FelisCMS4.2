...
;(function($, window, document, undefined)
{
	...
	window.notify = function(title, message, options, rotate)
	{
		...
		else
		{
			...
			var classes = ['notification'].concat(settings.classes),
				...
				list = $('#'+listId),
				icon = (settings.icon && settings.icon.length > 0) ? '<img class="notification-icon'+(settings.iconOutside ? ' outside' : '') + (settings.felis ? ' '+settings.felis : '')+'" src="'+settings.icon+'">' : '',
	...
	window.notify.defaults = {
		 ...
		 * @var boolean
		 */
		textOneSimilar: 'Jedno podobne zgłoszenie',
		 ...
		 * @var boolean
		 */
		textSeveralSimilars: '%d podobne zgłoszenia',
		 ...
		 * @var boolean
		 */
		textOneMore: 'Jedno podobne powiadomienie',
		 ...
		 * @var boolean
		 */
		textSeveralMore: '%d podobne powiadomienia',
