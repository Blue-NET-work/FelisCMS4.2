                        ...
                        {foreach from=$pages item=item name=pages}
                            <!-- wpis -->
                                <tr>
                                    <td>{$item.id}</td>
                                    <td>{$item.name}</td>
                                    <td nowrap="nowrap"><a href="#" data-href="{$base_url}../nagrody/nagroda/{$item.id}.html" class="preview with-tooltip" title="{lang line="default_sidebar_access_preview"}" data-title="{$item.name}">nagrody/nagroda/{$item.id}.html</a></td>
                                    {if $service_multilang == 'TRUE'}<td>{$item.lang}</td>{/if}
                                    <td class="actions">
                                        <a href="{$base_url}nagrody/edit/{$item.id}.html" title="{lang line="default_edit"}" class="felisEdit with-tooltip glyphicon glyphicon-pencil"></a>
                                    {if $item.active == 0}
