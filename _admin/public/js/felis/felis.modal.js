...
;(function($, window, document)
{
	...
	$.modal = function(options)
	{
		...
		$.each(settings.actions, function(name, config)
		{
			...
			// Insert
			$('<li'+(config.color ? ' class="'+config.color+'-hover"' : '')+'><a href="javascript:void(0)" title="'+name+'">'+name+'</a></li>').appendTo(actionsBlock).children('a').click(function(event)
			{
				event.preventDefault();
