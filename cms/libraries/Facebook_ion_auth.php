...
class Facebook_ion_auth {
	...
	*/
	...
	public function __construct() {
		...
		// config
		$this->app_id = "445668822246035"; // your app id
		$this->app_secret = "a4beaa788cbeb870b60772a22da72baa"; // your app secret key
		$this->my_url = site_url('auth/loginfacebook'); // url to redirect back from facebook
		...
		if(!$code) {
			...
			if($this->CI->session->userdata('state') && ($this->CI->session->userdata('state') === $this->CI->input->get('state'))) {
				...
				if(!$this->CI->ion_auth_model->identity_check($user->email)){
					...
					$login = $this->CI->ion_auth->login($user->username, 'facebookdoesnothavepass123^&*%', 1);
				}
